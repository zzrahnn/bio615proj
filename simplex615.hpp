...
template <class F> simplex615<F>::simplex615(double* p, int d) : dim(d) { // set dimension
...
bool simplex615<F>::updateSimplex(F& foo, double scale) {
    ...
    double fNext = foo(nextPoint); if ( fNext < Y[idxHi] ) { // update only maximum values (if possible)
